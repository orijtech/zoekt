...
// SearcherBloomChecker is a temporary hack to allow recomputing bloom filters
// before we invalidate the .zoekt files and move to the next version.
type SearcherBloomChecker interface {
	MissingBloomFilter() bool
	RebuildBloomFilter(string)
}
